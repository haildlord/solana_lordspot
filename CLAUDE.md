...
- **Workers** (`src/workers/index.ts`) — three BullMQ consumers chained by queue, plus the Base confirmer and the settlement worker:
  1. `webhookIngestWorker` — re-fetches the tx from Solana RPC by signature. **Chain is the only source of truth, unconditionally**: a signature not visible on-chain throws so BullMQ retries (6 attempts, exponential from 3s), and anything past that is still recovered by the cron backup indexer, which reads the vault ATA's real signature history. There is deliberately **no fallback to the webhook payload's own logs** — that fallback existed once, gated only on `NODE_ENV !== 'production'` (an exact lowercase compare, so `Production`/`prod`/`staging`/unset all left it open), and let anyone holding the webhook secret mint `RelayOrder`s for tickets nobody paid for, spending real Base vault USDC. Do not re-introduce it as a dev convenience; the backup indexer already covers the legitimate case. Checks for the `BuyTicket` instruction, forwards to the next queue.
...
**Payout** is the two-signature voucher model: `POST /v1/claims/voucher` (`backend/src/routes/claims.ts`) sums a wallet's unbound `CLAIMED_ON_BASE` tickets, binds them to a new `PayoutClaim` row, and has `solanaService.buildClaimVoucher` build+partially-sign a `claim_winnings` transaction. Note free-tier wins **are included** (v1 stopgap — they're paid as cash since the redeem-as-a-ticket flow is unbuilt; `isFreeTicketTier` stays set on the row so v2 can find them again), and `/summary` uses the identical query, so the two always agree — keep it that way when editing either: the partner SDK (which lives on the separate `sdk_v1` branch) verifies a voucher's amount against an independent `/summary` read before signing, so any drift between the two queries breaks claiming outright. Claims are all-or-nothing: nothing in the request body reaches the voucher except the wallet address. The frontend (`frontend/src/solana/claimVoucher.ts`) only needs the user to counter-sign and broadcast. On-chain, `claim_winnings` requires *both* signatures (user proves consent/ownership, admin authorizes the exact amount — neither alone can move funds) and pays into the signer's own canonical ATA only. **The user is the fee payer** (`feePayer: user`), so the transaction signature is NOT known at issuance — which is why `backend/src/workers/payoutConfirmer.ts` discovers landed claims via a *time-bounded chain scan* of admin signature history (`solanaService.findLandedClaims`), never a cursor/watermark (which could only move forward and permanently hide a missed tx) and never a frontend callback. It resolves each claim to `CONFIRMED` (tickets → `PAID_OUT_ON_SOLANA`), `FAILED`, or `EXPIRED` (past `lastValidBlockHeight` + a buffer) — the latter two release the bound tickets so a fresh voucher can be issued, and it refuses to expire anything on an incomplete scan. One-live-voucher-per-wallet is enforced by `/voucher` simply re-returning an existing `PENDING` claim instead of creating a second one, which is what makes double-payment structurally impossible.
