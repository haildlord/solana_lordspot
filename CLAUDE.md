...
`Ticket.winStatus` (Prisma enum `WinStatus`): `DRAW_PENDING → LOST | WON_UNCLAIMED | WON_FREE_TICKET → CLAIMED_ON_BASE → PAID_OUT_ON_SOLANA`. The last two states (harvest + Solana payout) are architecture-only so far — not yet implemented; only grading (`DRAW_PENDING` → the three post-draw states) is built.
...
**Designed but not built**: harvest (relayer batches `claimWinnings` on the Base vault — needs a new `onlyRelayer` vault function, doesn't exist yet — pulling NFT-owned winnings into the vault; net-per-ticket is read from the `TicketWinningsClaimed` event, not recomputed) and payout (Solana-side pull-claim via a new admin `credit_winnings` + user-signed `claim_winnings` instruction pair, chain-verified so a backend compromise can't move funds — deliberately *not* a backend-pushed transfer, for consent/address-safety/blast-radius reasons). The Solana purchase pot and the Base harvest pot are expected to roughly self-balance; only the imbalance needs bridging (CCTP, not a wrapped bridge), with an alert threshold for jackpot-sized tail wins pending manual liquidity rebalance.
